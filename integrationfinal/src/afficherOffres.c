...
void afficherPrestationOffre(GtkWidget *liste)
{
	...
	if (store==NULL)
	{
		...
		column = gtk_tree_view_column_new_with_attributes(" numero de l'offre", renderer, "text",NUM, NULL);
		gtk_tree_view_append_column(GTK_TREE_VIEW(liste),column);
		
		renderer = gtk_cell_renderer_text_new ();
		column = gtk_tree_view_column_new_with_attributes(" offre pour", renderer, "text",OFFRE, NULL);
		gtk_tree_view_append_column(GTK_TREE_VIEW(liste),column);
