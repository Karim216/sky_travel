                                        ...
                                        gpointer         user_data)
{
	...
	combo=lookup_widget(obj,"type");

	spin4=lookup_widget(obj,"joureserv");
	spin5=lookup_widget(obj,"moisreserv");
	spin6=lookup_widget(obj,"anneereserv");

	strcpy(type1,gtk_combo_box_get_active_text(GTK_COMBO_BOX(combo)));

	jour=gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin4));
	mois=gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin5));
	annee=gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(spin6));
	...
	f=fopen("offres.txt","r");
	rel=fopen("rela.txt","w");

	if(f!=NULL && rel!=NULL)
        {
		while(fscanf(f,"%s %s %s %s %s\n",id,type, date,heure,prix)!=EOF)
                  {
			if(strcmp(type,type1)==0 && strcmp(date,date1)==0)
                        {
				fprintf(rel,"%s %s %s %s %s\n",id,type, date,heure,prix);
			}
                 }
